...
public class RoundStartState : StateBase
{
	...
	public override void OnEnter()
	{
		...
		_wordListBanner = new BannerText(bannerHeight, bannerHeight*2+bannerGap,
		...
		{
			_modifierBanner = new BannerText(bannerHeight, bannerHeight*3+bannerGap*2, "Modifiers:",false, "None");
		}
		else
		{
			_modifierBanner = new BannerText(bannerHeight, bannerHeight*3+bannerGap*2, "Modifiers:",false, _machine.Game.CurrentRound.Test.Modifiers.Select(x=>x.GetModifierName()).ToArray());
		}
	...
	public override void Tick(float delta)
	{
		...
		if (_modifierBanner.ExitTween.PercentageComplete > 0.5f && !_wordListBanner.ExitTween.IsComplete)
		{
			_wordListBanner.ExitTween.Tick(delta);
		}
		
		if (_modifierBanner.ExitTween.PercentageComplete > 0.5f && _wordListBanner.ExitTween.PercentageComplete > 0.5f && !_demonNameBanner.ExitTween.IsComplete)
		{
		...
		//done
		if (_modifierBanner.IsComplete && _wordListBanner.ExitTween.IsComplete && _demonNameBanner.ExitTween.IsComplete)
		{
